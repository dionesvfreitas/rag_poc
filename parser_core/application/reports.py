...
def audit_warnings(
    ...
    *,
    ...
    if figures_saved > figures_linked_to_chunks:
        add_warning(
            warnings,
            f"{figures_saved - figures_linked_to_chunks} saved figure asset(s) are not linked to chunks.",
        )
...
def build_report(parsed_document, chunks, warnings=None, parser_name="docling"):
    ...
    blocks_without_section = sum(1 for block in blocks if not block.section_path)
    chunks_without_section = sum(1 for chunk in chunks if not chunk.section_path)
